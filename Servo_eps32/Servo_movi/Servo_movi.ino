...
const int PASO_SERVO =
    6; // Grados que se moverá el servo por cada comando (antes era 8)
const int DELAY_PASO =
    15; // Tiempo (ms) por cada grado de movimiento en tracker
...
bool movimientoEnProceso =
    false; // Bandera para indicar si el servo está en movimiento
...
int pivotIncrement = 6;           // Paso de incremento/decremento
unsigned long pivotInterval = 50; // Tiempo entre pasos de rampa (ms)
unsigned long pivotHoldTime =
    2000; // Tiempo para mantener velocidad máx/mín (ms)
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.setTimeout(4); // Ajustar el tiempo de espera para Serial

  velocidad_h = 90;
  velocidad_v = 90;

  // Escribir la posición inicial (90, 90) ANTES de hacer attach para evitar
  // tirones bruscos
  ...
  analogWrite(
      ENA,
      128); // PWM de 50% de ciclo de trabajo (ajusta el valor si es necesario)
...
void moverServoSuavemente(Servo &servo, int *anguloActual, int incremento,
                          int limite) {
  int anguloObjetivo = *anguloActual + incremento;

  // Limitar el ángulo dentro de los límites especificados
  if (incremento > 0 && anguloObjetivo > limite)
    anguloObjetivo = limite;
  if (incremento < 0 && anguloObjetivo < limite)
    anguloObjetivo = limite;
...
void centrarServos() {
  while (velocidad_h != 90 || velocidad_v != 90) {
    if (velocidad_h < 90)
      velocidad_h++;
    else if (velocidad_h > 90)
      velocidad_h--;

    if (velocidad_v < 90)
      velocidad_v++;
    else if (velocidad_v > 90)
      velocidad_v--;
